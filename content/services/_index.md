---
title: "Services"
description: "Professional OSINT research services — due diligence, fact-checking, and compliance research."
layout: "services"
---

Lucerna offers selective OSINT research services to organizations and individuals who need rigorous, evidence-based intelligence. Every engagement follows our published methodology — transparent sources, reproducible analysis, graded confidence.

We work with law firms, compliance departments, journalists, and investors.

## OSINT Due Diligence

Background checks on companies, individuals, and transactions using exclusively open sources. Corporate structure mapping, beneficial ownership tracing, reputation analysis, digital footprint assessment.

Typical scope: counterparty verification, pre-investment screening, M&A target profiling.

## Fact-Check & Verification

Independent verification of specific claims, statements, or published information. Source triangulation, timeline reconstruction, metadata analysis.

Typical scope: media claims verification, witness statement cross-referencing, document authenticity assessment.

## Compliance Research

Regulatory analysis for organizations operating across complex jurisdictions. EU AI Act readiness, GDPR compliance auditing, cross-border data transfer assessments.

Typical scope: regulatory gap analysis, compliance documentation review, policy recommendations.

---

## Selected Work

Our published investigations demonstrate the depth and rigor of our approach. Client engagements follow the same methodology with confidential reporting.

**Career Reconstruction** — Reconstruction of a professional timeline using open sources, cross-referencing corporate registries, social media archives, and public records across multiple countries. [Read the published version]({{< relref "/investigations/osint-career-reconstruction-case" >}}).

**Corporate Due Diligence** — Multi-jurisdictional verification of a technology company's public claims. Cross-referenced corporate registries, financial filings, and domain records across four countries. Delivered structured evidence report with confidence grading for each finding.

**Regulatory Compliance Audit** — Gap analysis of a mid-size organization's data handling practices against GDPR and EU AI Act requirements. Mapped data flows, identified non-conformities, and produced a remediation roadmap with prioritized action items.

---

## Engagement

Each project is scoped individually. We do not operate on fixed retainers or standardized packages — every engagement is tailored to the question at hand.

To discuss a potential project, write to [contact@folkup.app](mailto:contact@folkup.app). Include a brief description of what you need investigated or verified. We typically respond within 48 hours.

All communications are treated as confidential. We do not disclose client identities or project details. See our [Privacy Policy]({{< relref "/legal/privacy-policy" >}}).
